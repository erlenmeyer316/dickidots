...
new_profile() {
  ...
  if ! file_exists "${NEW_PROFILE_PATH}/${SETUPS_PKGLIST}"; then
    ...
    if [[ "$REPLY" == "y" ]] || [[ "$REPLY" == "" ]]; then
      ...
      file_create "${NEW_PROFILE_PATH}/${SETUPS_PKGLIST}"
    fi
  else
  ...
  if ! file_exists "${NEW_PROFILE_PATH}/${CONFIGS_PKGLIST}"; then
    ...
    read -r -p "[n/Y]: "
    if [[ "$REPLY" == "y" ]] || [[ "$REPLY" == "" ]]; then
      file_create "${NEW_PROFILE_PATH}/${CONFIGS_PKGLIST}"
    fi
  else
  ...
  if ! file_exists "${NEW_PROFILE_PATH}/${PROFILE_DEPS_PKGLIST}"; then
    ...
    read -r -p "[n/Y]: "
    if [[ "$REPLY" == "y" ]] || [[ "$REPLY" == "" ]]; then
      file_create "${NEW_PROFILE_PATH}/${PROFILE_DEPS_PKGLIST}"
    fi
  else
  ...
  if ! file_exists "${NEW_PROFILE_PATH}/${INSTALL_DEPS_PKGLIST}"; then
    ...
    read -r -p "[n/Y]: "
    if [[ "$REPLY" == "y" ]] || [[ "$REPLY" == "" ]]; then
      file_create "${NEW_PROFILE_PATH}/${INSTALL_DEPS_PKGLIST}"
    fi
  else
