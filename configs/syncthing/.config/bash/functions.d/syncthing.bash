...
st_info() {
  echo "── System ───────────────────────────────────────"
  local sys
  sys=$(_st_curl /rest/system/status) || return 1
  echo "$sys" | jq -r '"Device ID : " + .myID, "Version   : " + .version'

  echo ""
  echo "── Connections ──────────────────────────────────"
  local conn
  conn=$(_st_curl /rest/system/connections) || return 1
  echo "$conn" | jq -r '
  ...
  echo "── Folders ──────────────────────────────────────"
  local folders
  folders=$(_st_curl /rest/config/folders) || return 1
...
st-pending() {
  ...
  local devices
  devices=$(_st_curl /rest/cluster/pending/devices) || return 1
  if [[ "$devices" == "{}" ]]; then
    echo "  none"
  else
  ...
  local folders
  folders=$(_st_curl /rest/cluster/pending/folders) || return 1
  if [[ "$folders" == "{}" ]]; then
    echo "  none"
  else
